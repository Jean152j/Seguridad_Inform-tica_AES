document.addEventListener("DOMContentLoaded", async () => {
	...
	$botonEncriptar.onclick = async () => {
		...
		if (!contraseña) {
			return alert("No ha ingresado ninguna contraseña");
		}
		const textoPlano = $informacionEncriptar.value;
		if (!textoPlano) {
			return alert("No ha ingresado un texto para encriptar");
		}
	...
	$botonDesencriptar.onclick = async () => {
		...
		if (!textoCifradoBase64) {
			return alert("No ha ingresado un texto para desencriptar");
		}
		try {
			const desencriptado = await desencriptar(contraseña, textoCifradoBase64);
			$resultadoDesencriptacion.value = desencriptado;
		} catch (e) {
			$resultadoDesencriptacion.value = "Error al desencriptar: " + e.message + " Revise la contraseña y la cadena ingresada";
		}
	};
